pub mod btrie;
pub mod proto;
pub mod util;

use btrie::BTrieRoot;
use lazy_static::lazy_static;
use proto::data::{Translation, VerseKey, VerseText};
use regex::Regex;
use std::collections::HashMap;
use util::InterIter;

pub use util::Config;

const MAX_RESULTS: usize = 20;

pub struct VersearchIndex {
    btrie: BTrieRoot<VerseKey, Translation>,
}

lazy_static! {
    static ref RE: Regex = Regex::new(r"[\s.]+").unwrap();
}

impl VersearchIndex {
    #[allow(clippy::new_without_default)]
    pub fn new() -> VersearchIndex {
        VersearchIndex {
            btrie: BTrieRoot::new(),
        }
    }

    pub fn from_index_bincode(bytes: Vec<u8>) -> VersearchIndex {
        VersearchIndex {
            btrie: bincode::deserialize(&bytes).unwrap(),
        }
    }

    pub fn insert_doc(&mut self, id: &VerseKey, doc: &HashMap<Translation, String>) {
        self.btrie.insert_doc(id, doc);
    }

    pub fn search(
        &self,
        text: &str,
    ) -> Option<
        Vec<(
            proto::data::VerseKey,
            std::collections::HashMap<proto::data::Translation, usize>,
        )>,
    > {
        // Step 1: collect all matches
        let mut matching_iters = Vec::new();
        for word in RE.split(&text.to_uppercase()) {
            if let Some(iter) = self.btrie.iter_prefix(word) {
                matching_iters.push(iter);
            }
        }
        // // Step 2: find all common matches
        let results: Vec<(
            proto::data::VerseKey,
            std::collections::HashMap<proto::data::Translation, usize>,
        )> = InterIter::new(matching_iters, |(vk1, _), (vk2, _)| vk1.cmp(&vk2))
            // .copied()
            .take(MAX_RESULTS)
            .collect();
        // // Step 3: We made it!
        Some(results)
    }

    pub fn index_to_bincode(&self) -> Result<Vec<u8>, bincode::Error> {
        bincode::serialize(&self.btrie)
    }
}
